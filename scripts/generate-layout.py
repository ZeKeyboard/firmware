...
// This file is generated by generate-layout.py. Do not edit manually.
...
def parse_json_layout(json_layout) -> List[List[KeyDescription]]:
    curr_y = 0
    rows = []
...
def write_cpp(rows: List[List[KeyDescription]], output_path: str):
    total_num_keys = sum([len(row) for row in rows])
    rows_str = ",\n    ".join([",\n    ".join(
        [key.to_cpp() for key in row]) for row in rows])
    cpp = SKELETON.format(total_num_keys=total_num_keys, rows=rows_str)
...
def main():
    ...
    rows = parse_json_layout(json_layout)
    write_cpp(rows, args.output)
