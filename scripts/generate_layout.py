...
class KeyDescription:
    def __init__(self, row: int, col: int, x: float, y: float, height: float, width: float, default_key: Optional[str]):
        self.row: int = row
        self.col: int = col
        self.x: float = x
        self.y: float = y
        ...
        self.default_key: str = default_key if default_key is not None else "0x0";
    ...
    def to_cpp(self) -> str:
        return f"KeyDescription {{ {self.row}, {self.col}, {self.x}, {self.y}, {self.height}, {self.width}, {self.default_key} }}"
...
def extract_default_key(text: str) -> Optional[str]:
    ...
    key_first_half = keycodes.KEY_CODES.get(text_col_split[0].strip(), None)
    if key_first_half is not None:
        return key_first_half
    return keycodes.KEY_CODES.get(text_col_split[1].strip(), None)
    ...
    for row_num, row in enumerate(json_layout):
        ...
        for item in row:
            if isinstance(item, dict):
                ...
                default_key = extract_default_key(item)
                key = KeyDescription(
                    row=row_num,
                    col=col_num,
                    x=curr_x,
